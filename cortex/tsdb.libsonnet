{
  ...
  _config+:: {
    ...
    cortex_compactor_cleanup_interval: '15m',  // Default value, we want to pass the same to the -blocks-storage.bucket-store.sync-interval

    // Enable use of bucket index by querier, ruler and store-gateway.
    // Bucket index is generated by compactor from Cortex 1.7, there is no flag required to enable this on compactor compactor.
